...
1. 테이블 생성명령
  ...
  CREATE TABLE 테이블명 (
    ...
    - 기본키설정명 : 기본키설정에 부여한 이름으로 한 테이블 중복사용할 수 없음.
    - 외래키설정명1 : 외래키설정에 부여한 이름으로 중복사용할 수 없음.
    - REFERENCES 테이블명 : 부모테이블명 
    - REFERENCES 테이블명 (칼럼명) :  부모테이블에서 사용한 컬럼명
    - DELETE ON CASCADE : 부모테이블에서 특정행(ROW) 삭제시 자식테이블의 자료부터 삭제하고 부모 자료 삭제 허용
    
    
    SELECT TO_CHAR(SYSDATE, 'YYYY-MM-DD HH24:MI:SS') FROM DUAL; <==현재시간 확인하는법
...
3. ALTER 명령
   ...
   3) 컬럼DML 데이터 타입 또는 크기 변경
    ...
    ALTER TABLE 테이블명 ADD(CONSTRAINT 외부키설정명 FOREIGN KEY(컬럼명)
     REFERENCES 테이블명(컬럼명))
    사용예시) SITE테이블의 기본키(SITE_ID)를 설정하시오.
