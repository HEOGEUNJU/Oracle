...
--사용예--
  년도와 월을 입력 받아 해당 기간에 가장 많은 구매액을 기록한 회원정보를 조회하시오
  Alias는 회원번호, 회원명, 구매금액, 주소이다.
  (프로시져)
    입력 : 년도와 월 
    처리할 내용 : CART테이블에서 최대 구매 금액을 기록한 회원번호, 구매액 합계
    출력 : 회원번호, 구매액 합계
  CREATE OR REPLACE PROCEDURE PROC_CART01(
    P_PERIOD IN VARCHAR2, -- 년월 입력, 크기 쓰면 안됨
    P_MID OUT MEMBER.MEM_ID%TYPE, -- 회원번호 출력
    P_SUM OUT NUMBER) -- 구매금액 합계 출력
  IS           
  BEGIN
...
--사용예--
  직업이 '자영업'인 회원번호를 입력 받아 2020년 상반기(1-6월) 구매현황을 출력하는 프로시져를 작성하시오
  Alias는 회원번호, 회원명, 구매금액합계

--사용예--
  다음 자료를 판매한 경우 매출처리를 프로시져로 작성하시오  판매-> 재고처리-> 마일리지
  판매자료
  --------------------------------------------------
  구매회원번호     날짜         상품코드        수량
  --------------------------------------------------
