...
CONNECT BY [NOCYCLE][PRIOR 계층형 구조 조건];
 ...
 . 마지막 컬럼에 나열한 LEVEL컬럼은 레벨 의사컬럼(LEVEL Pseudocolumn)이라고 하는데 계층형 정보를 표현할 때 그 계층의 레벨을 표현
 . 이 레벨 의사컬럼을 다양하게 응용 - 가장 많이 응용하는 방법이 레벨별로 들여 쓰기를 하여 좀
   더 직관적으로 데이터를 표현
 . PRIOR연산자
 - PRIOR키워드는 오직 계층형 쿼리에서만 사용하는 오라클 SQL연산자이다.
   키워드라고 표현했지만 실제로는 CONNECT BY절에서 등호(=)와 동등한 레벨로 사용되는 연산자이며
   CONNECT BY절에서 해당 컬럼의 부모로우를 식별하는데 사용된다.
...
사용예)
 ...
 FROM bom
 START WITH parent_id IS NULL --루트노드를 지정, 제일 꼭대기에 있는 것이다.(트리구조)
 CONNECT BY PRIOR item_id = parent_id;--부모와 자식노드들간의 관계를 연결 //왼쪽은 자식 오른쪽은 부모


 SELECT LPAD(' ', 2*(LEVEL-1)) || item_name as item_names,-- 거의 공식이라고 생각, 계층으로 출력될 수 있다.
 item_id,
 parent_id
 FROM bom
 ...
 select level,--출력햇을때 NULL값을 가지고 있는 게 상위코드부서임
