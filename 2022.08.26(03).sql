2022-0826-03)순위함수
 ...
 1) RANK()
 . 일반적인 순위함수
 . 같은 값이면 같은 순위를 부여하고 다음 순위는 '현순위+동일순위 개수'로 부여
 ...
 사용예) 회원테이블에서 회원번호,회원명,마일리지,순위를 조회하시오
        ...
        SELECT MEM_ID AS 회원번호, 
               ...
               RANK() OVER(ORDER BY MEM_MILEAGE DESC,  --마일리지먼저 적용 후
            (EXTRACT(YEAR FROM SYSDATE)-EXTRACT(YEAR FROM MEM_BIR)) DESC) AS 순위 --나이 적용 마일리지를 가지고 등수를 구분하지 못할때

        FROM MEMBER;

 2) DENSE_RANK()
  . 순위를 구하는 기능은 RANK()와 동일
  . 동일 순위가 복수개 발생하더라도 다음 순위의 현재순위 바로 다음 순위로 부여
...
--            (EXTRACT(YEAR FROM SYSDATE)-EXTRACT(YEAR FROM MEM_BIR)) DESC) AS 순위 --나이 적용 마일리지를 가지고 등수를 구분하지 못할때
 ...
 4) 그룹별 순위
  . 자료를 그룹으로 분류하고 각 그룹 내에서 특정 컬럼의 값을 기중으로 순위를 부여
  . PARTITION BY 예약어로 그룹을 구성함
 
 (사용형식)
  RANK() OVER(PARTITION BY 컬럼명p1[,컬럼명p2,...] ORDER BY 컬럼명b1 [DESC|ASC] [,컬럼명b2 [DESC|ASC],...])
  - SELECT문의 SELECT절에 사용
  
 사용예)사원테이블에서 부서별로 급여에 따른 순위를 부여하시오
        Alias는 사원번호, 사원명, 부서번호, 급여, 순위이며 순위는 급여가 많은 사람 순으로
